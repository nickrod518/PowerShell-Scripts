...
# Log everything
$LogDirectory = (New-Item -ItemType Directory "C:\powershell-scripts\Exchange\Logs" -Force).FullName
...
$Gal | ForEach-Object {
    ...
    if (Get-MailContact -Identity $_.Name -ErrorAction SilentlyContinue) {
        Write-Verbose "Contact $($_.Name) already exists."
    } else {
        New-MailContact `
    ...
    Set-MailContact `
        ...
        -DisplayName $_.DisplayName `
        -Alias $_.Alias `
        -WindowsEmailAddress $_.WindowsLiveID `
