...
function Get-File {
    ...
    if ($Type -eq 'ltccab') {
        $OpenFileDialog.InitialDirectory = "\\$Computer\C$"
        $OpenFileDialog.Filter = "LTC Cab File (*.ltccab)| *.ltccab"
    } else {
        $OpenFileDialog.InitialDirectory = "C:\MSIShare"
...
function Install-Setup {
    Param (
		...
		[Parameter(Mandatory=$false)] $DBUpdate
	)
	...
	Invoke-Command -ComputerName $Computer -AsJob -JobName "Install-$($Setup.BaseName)" -ScriptBlock {
        ...
        if ($Extension -eq 'msi') {
            ...
            # If it's a database update, we need to kill the database select prompt in order to proceed
            if ($DBUpdate) {
                # Start the install as a background process
                $Result = Start-Process "$Temp\$SetupName.exe" -ArgumentList "/s /f1`"$Temp\$SetupName.iss`" /f2`"$Temp\$SetupName.log`"" -PassThru
                $Process = Get-Process -Name $SetupName
                while (-not $Process.HasExited) {
                    try {
                        Stop-Process -Name AHT.Tools.LTCDBUpdate -ErrorAction SilentlyContinue
                    } catch { }
                    Start-Sleep -Seconds 1
                }
            } else {
        ...
        Return $ExitCode
    } -ArgumentList ($Setup.Name, $Computer, $MSIOptions, $DBUpdate)
}
...
Function Deploy {
    ...
    foreach ($Job in $Jobs) {
        ...
        if ($DBUpdate) {
            if ($LTCCAB) {
                if (Test-Path "\\$Computer\C$\Program Files (x86)\American Healthtech\LTCNET\Maint_Updates\$LTCCAB") {
                    Write-Log -Message "DB update validated." -Component $Computer -LogFile $Log
                } else {
                    Write-Log -Message "DB update not validated." -Type 3 -Component $Computer -LogFile $Log
                }
            } else {
                Write-Host "`nSelect an ltccab file to use for validation..."
                try { $LTCCAB = (Get-File -Type 'ltccab' -Computer $Computer).Name } catch { Exit }
                $LTCCAB
                Write-Log -Message "DB update validated." -Component $Computer -LogFile $Log
            }
        }
...
DBUpdate: Some updates initiate a prompt at the end of the install process for the user to select databases to update, enable this to kill that prompt so the install can continue
#>
Deploy -ServerList $ServerList -ThrottleLimit 5 -DBUpdate $true -MSIOptions "AHT_DEPLOYMODE='N' ALLUSERS=1 REBOOT=ReallySuppress"
