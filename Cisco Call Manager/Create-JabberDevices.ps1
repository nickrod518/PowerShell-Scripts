...
function Process-Location($location, $locationOU) {
    Write-Host "Processing $location..."

    $locationNoSpaces = $($location.Replace(' ', ''))
    ...
    $ADLocationUsers = Get-ADUser -SearchBase $locationOU -Properties sAMAccountName, ipPhone, telephoneNumber, distinguishedName, sAMAccountName -Filter *
    ...
    Write-Host "Finished processing $location!`n" -ForegroundColor Green
}

# exclude these locations
$badOffices = @('Office5', 'Office6')
# get all the locations
$offices = Get-ADOrganizationalUnit -SearchBase 'OU=Offices,DC=CORP,DC=LOCAL' -Filter * | select Name, distinguishedName | Sort-Object Name | Where-Object { $badOffices -notcontains $_.Name }
# process each office
foreach ($office in $offices) { Process-Location $office.Name $office.distinguishedName }
